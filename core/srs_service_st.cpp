...
srs_error_t srs_tcp_bind_connect(string server, int port, string bindserver, int bindport, srs_utime_t tm, srs_netfd_t* pstfd)
{
    ...
    *pstfd = NULL;
    ...
    if ((err = srs_fd_reuseaddr(sock)) != srs_success) {
        return srs_error_wrap(err, "set reuseaddr");
    }

    // 支持多个进程或者线程绑定到同一端口 可不设置
    if ((err = srs_fd_reuseport(sock)) != srs_success) {
    ...
    addrinfo* bindr  = NULL;
    char bindsport[8];
    snprintf(bindsport, sizeof(bindsport), "%d", bindport);
    SrsAutoFree(addrinfo, bindr);
    addrinfo bindhints;
    memset(&bindhints, 0, sizeof(bindhints));
    bindhints.ai_family = AF_UNSPEC;
    bindhints.ai_socktype = SOCK_STREAM;
    if(getaddrinfo(bindserver.c_str(), bindsport, (const addrinfo*)&bindhints, &bindr)) {
        srs_close_stfd(stfd);
        return srs_error_new(ERROR_SYSTEM_IP_INVALID, "get address info");
    }
    if (::bind(sock, bindr->ai_addr, bindr->ai_addrlen) == -1) {
        srs_close_stfd(stfd);
...
srs_error_t SrsTcpClient::bind_and_connect(string bindserver, int bindport)
{
    srs_error_t err = srs_success;
    ...
    srs_assert(stfd == NULL);
    if ((err = srs_tcp_bind_connect(host, port, bindserver, bindport, timeout, &stfd)) != srs_success) {
        return srs_error_wrap(err, "tcp: connect %s:%d to=%dms  bind:[%s:%d]", host.c_str(), port, srsu2msi(timeout), bindserver.c_str(), bindport);
    }
